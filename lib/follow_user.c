...
{
	char url[128];
	sprintf(url, "%s://%s/api/friendships/create.xml", account.protocol, account.server);
	...
	char screen_name_to_follow[100];
	sprintf(screen_name_to_follow, "screen_name=%s", screen_name);

	curl_easy_setopt(curl, CURLOPT_POSTFIELDS, screen_name_to_follow);
